...
for (let r = 0; r < 10; r++) {
  ...
  if (res.unplacedCount <= 10) {
    console.log(`\nTesting repair on restart #${r} with unplaced = ${res.unplacedCount}...`);
    const repaired = gen._repairSchedule(res.slots, res.unplacedUnits, classes, teachers, subjects);
    console.log(`Repair result: success=${repaired.success}, unplacedCount=${repaired.unplacedCount}, slots=${repaired.slots?.length}`);
    
    // Check remaining unplaced
    for (const u of (repaired.unplacedUnits || [])) {
      ...
      console.log(`  Unrepaired: [${c}] ${s} (${t}) dur=${u.duration}`);
    }
    break;
  }
}
