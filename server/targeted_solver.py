...
print(f"Starting Score: {cur_tot} (Hard={cur_h}, Soft={cur_s}, P8={cur_p8}, DD={cur_dd}, Adj={cur_adj})", flush=True)

# Greedy Hill Climbing with Unit Swaps
improved = True
pass_num = 0
...
while improved:
    ...
    for c_id, c in classes.items():
        ...
        # Test all pairs of same-size units
        best_delta = 0
        best_swap = None
        ...
        for i in range(len(units)):
            for j in range(i + 1, len(units)):
                ...
                if h == 0 and p8 == 0:
                    ...
                    if delta < best_delta:
                        best_delta = delta
                        best_swap = (uA, uB, revert, tot, dd, adj)
        ...
        if best_swap:
            uA, uB, revert, new_tot, new_dd, new_adj = best_swap
            # Apply best swap
            dA, pA = uA['day'], uA['periods']
            dB, pB = uB['day'], uB['periods']
            for s in uA['slots']:
            ...
            cur_tot = new_tot
            improved = True
            print(f"  [{c_name}] Improved by {best_delta}: {uA['day']} P{uA['periods']} <-> {uB['day']} P{uB['periods']} -> Score={cur_tot} (DD={new_dd}, Adj={new_adj})", flush=True)

final_tot, final_h, final_s, final_p8, final_dd, final_adj = fast_eval(cur_slots)
print(f"\nOptimization Finished: Score={final_tot} (Hard={final_h}, Soft={final_s}, P8={final_p8}, DD={final_dd}, Adj={final_adj})", flush=True)
