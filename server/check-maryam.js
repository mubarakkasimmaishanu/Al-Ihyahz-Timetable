...
console.log('=== Checking M. Maryam Allocations & Timetable Slots ===');

const teacher = db.prepare("SELECT * FROM teachers WHERE name = 'M. Maryam'").get();
console.log('Teacher Record:', teacher);

if (!teacher) {
  console.error('Teacher M. Maryam not found!');
  process.exit(1);
}
...
console.log(`\n--- Placed Slots (Total: ${slots.length} / 18) ---`);
console.table(slots);
