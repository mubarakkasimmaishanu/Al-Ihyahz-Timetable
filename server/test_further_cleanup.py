...
tot, h, s, p8, dd, adj, issues = evaluate(cur_slots)
print(f"Base: Score={tot}, Hard={h}, P8={p8}, DD={dd}, Adj={adj}")

# Search 2-step unit swaps in SS 1 and SS 3
for target_class in ['SS 1', 'SS 3']:
    cid = next(i for i, c in classes.items() if c['name'] == target_class)
    units = get_class_units(cur_slots, cid)
    print(f"\nSearching moves for {target_class} ({len(units)} units)...")
    ...
    for i in range(len(units)):
        for j in range(i + 1, len(units)):
            ...
            for slt in uA['slots']:
                ...
                revert.append((slt, slt['day'], slt['period_index']))
            ...
            tot2, h2, s2, p8_2, dd2, adj2, iss2 = evaluate(cur_slots)
            if h2 == 0 and p8_2 == 0 and (adj2 < adj or dd2 < dd):
                print(f"  FOUND IMPROVEMENT in {target_class}: {dA} P{pA} <-> {dB} P{pB} => Adj={adj2}, DD={dd2}, Score={tot2}")

            for slt, d_orig, p_orig in revert:
                slt['day'] = d_orig
