...
# Apply Rotation 3:
# A: SS 2 Wed P3 (AGR) -> Thu P8
# B: SS 2 Thu P8 (DPR) -> Wed P8
# C: SS 2 Wed P8 (IRS) -> Wed P3
ss2_id = next(cid for cid, c in classes.items() if c['name'] == 'SS 2')
agr_id = next(sid for sid, s in subjects.items() if s['code'] == 'AGR')
dpr_id = next(sid for sid, s in subjects.items() if s['code'] == 'DPR')
sA = next(s for s in base_slots if s['class_id'] == ss2_id and s['day'] == 'Wednesday' and s['period_index'] == 3 and s['subject_id'] == agr_id)
sB = next(s for s in base_slots if s['class_id'] == ss2_id and s['day'] == 'Thursday' and s['period_index'] == 8 and s['subject_id'] == dpr_id)
sC = next(s for s in base_slots if s['class_id'] == ss2_id and s['day'] == 'Wednesday' and s['period_index'] == 8 and s['subject_id'] == irs_id)
...
tot, h, s, issues = evaluate(base_slots)
print(f"Result: Score={tot} (Hard={h}, Soft={s}, Issues={len(issues)})")
for iss in issues:
    print(f"  * {iss}")
...
for slot in base_slots:
    if slot['period_index'] == 8:
        p8_counts[teachers[slot['teacher_id']]['name']] += 1
for t_id, t in teachers.items():
    print(f"  {t['name'].ljust(20)}: {p8_counts[t['name']]} (target: {TARGET_P8.get(t['name'], 0)})")
