...
# Apply confirmed clean moves first:
...
def evaluate(slts):
    ...
    # 6. Double-double without break (Max 1 double per session per teacher)
    ...
    for c_id, cls in classes.items():
        for d in DAYS:
            ...
            # Group by period_index to handle simultaneous electives
            slots_by_p = defaultdict(list)
            for s in c_slots:
                slots_by_p[s['period_index']].append(s)
            
            periods = sorted(slots_by_p.keys())
            for idx in range(len(periods) - 1):
                p1 = periods[idx]
                p2 = periods[idx + 1]
                if p2 == p1 + 1:
                    teachers_p1 = {s['teacher_id']: s['subject_id'] for s in slots_by_p[p1]}
                    teachers_p2 = {s['teacher_id']: s['subject_id'] for s in slots_by_p[p2]}
                    common_teachers = set(teachers_p1.keys()).intersection(set(teachers_p2.keys()))
                    for t in common_teachers:
                        if teachers_p1[t] != teachers_p2[t]:
                            adj_count += 1
                            soft_penalty += 3000
                            issues.append(f"Adjacent diff subjects in {cls['name']} on {d}: {teachers[t]['name']} ({subjects[teachers_p1[t]]['code']} P{p1} -> {subjects[teachers_p2[t]]['code']} P{p2})")

    total = hard_penalty + soft_penalty
    return total, hard_penalty, soft_penalty, issues, p8_diff, adj_count

tot, h, s, issues, p8_diff, adj = evaluate(slots)
print(f"Initial Unit Solver State: Hard={h}, Soft={s}, P8_diff={p8_diff}, Adj={adj}, Total Issues={len(issues)}")
for iss in issues:
    print(f"  * {iss}")
